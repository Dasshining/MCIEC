...
Initialization: The initial population is generated by creating random viable
paths (random walks) originating from arbitrary nodes. Fitness Evaluation: The
compound cost function () is applied. Because the GA is designed to maximize
fitness, the cost is inverted (e.g., ). Selection: A tournament selection method
is utilized to choose parent chromosomes, favoring individuals with lower
overall network costs. Crossover: A single-point crossover mechanism swaps
entire routes between two parent solutions to generate offspring. Mutation: To
maintain genetic diversity and prevent premature convergence, offspring undergo
mutation with a specified probability. Mutation operators include extending a
route to an adjacent node, trimming the final stop, or entirely regenerating a
route within the chromosome. The algorithm iteratively evolves the population,
continuously plotting the most fit routes against the spatial data of
Guadalajara until a maximum generation threshold is reached, yielding the
optimized transit network configuration.
